...
public class Entry extends BaseEntry {
	...
	/**
	 * This is the entry point method. It is loaded for page where the meta tag
	 * is found.
	 */
	...
	private void loadOrdersWidgets() {
		...
		/*
		 * Order view page may contains number of <div/> with id in form
		 * reReserveItemDialog_0_1 where first digit is order item index and
		 * second one is inventory index for that order item index.
		 * 
		 * Try to get elements with all reasonable identifiers and install link
		 * widgets into them.
		 */
		...
		while (true) {
			...
			if (currentPanel != null) {
				...
				// if the inventory item index is zero, it means we have no more
				// div to find
	...
	/**
	 * Load list of parent accounts for a contact. Designed to use on view
	 * contact page.
	 */
	private void loadContactAccounts() {
		...
		// if required ID is found setup button [Assign Account] in subsection
		// title
	 ...
	 * 
	 * @param container
	 *            <code>RootPanel</code> that is root container for button
			...
			public void notifySuccess(String obj) {
				...
				request.setCallback(new RequestCallback() {
					...
					public void onResponseReceived(Request request, Response response) {
						// we don't expect anything from server, just reload the
						// list of contacts
	...
	private void loadOpportunityContacts() {
		...
		// if required ID is found setup button [Assign Contact] in subsection
		// title
	 ...
	 * @param container
	 *            <code>RootPanel</code> that is root container for button
	 * @param opportunityContacts
	 *            parent contacts list view.
	 */
	 ...
	 * 
	 * @param container
	 *            the container <code>RootPanel</code>
	...
	private void loadReReserveDialog(RootPanel container, Integer orderItemIndex, Integer oisgrIndex) {
		...
		final ProductReReservationForm window = new ProductReReservationForm(UtilUi.MSG.opentapsReReserveProduct(), facilities,
				getOrderId(), widgetParameters.get("orderItemSeqId_" + orderItemIndex.toString()), widgetParameters.get("inventoryItemId_"
						+ orderItemIndex.toString() + "_" + oisgrIndex.toString()), widgetParameters.get("shipGroupSeqId_"
						+ orderItemIndex.toString() + "_" + oisgrIndex.toString()), widgetParameters.get("quantity_"
						+ orderItemIndex.toString() + "_" + oisgrIndex.toString()));
		window.create();
	 ...
	 * 
	 * @return Possible values are <code>MY_VALUES</code> (or
	 *         <code>OrderLookupConfiguration.MY_VALUES</code>) and
	 *         <code>TEAM_VALUES</code> (or
	 *         <code>OrderLookupConfiguration.TEAM_VALUES</code>)
	 */
