...
/**
 * These are the fundamental infrastructure Resources used by opentaps,
 * including the ofbiz dispatcher, delegator, security objects and Hibernate
 * sessions for each delegator. Typically, all the OFBiz business tier
 * infrastructure can be obtained from their GenericDispatcher.
 * 
 */
public class Infrastructure {
	// singleton for multi-threads by liura 2013-09-29
	...
	private GenericValue systemUserLogin = null; // Sometimes, the system user
													// must be used by the
	...
	/**
	 * Gets the Hibernate <code>SessionFactory</code> object for the
	 * corresponding delegator. A Map of delegatorName to SessionFactory is
	 * maintained, and the first time a SessionFactory is requested for. A
	 * delegatorName, it is created in the the following way:
	 * <ul>
	 * <li>First, get the data source for the group helper (ie, "org.ofbiz" in
	 * entitygroup.xml) and that delegator</li>
	...
	public static synchronized SessionFactory getSessionFactory(String delegatorName) {
		Debug.logInfo("before hibernate SessionFactory size:" + getInstance().sessionFactories.size(), MODULE);
		Debug.logInfo("getSessionFactory(), delegatorName:" + delegatorName, MODULE);

		// add singleton 2013-09-29
		SessionFactory sessionFactory = getInstance().sessionFactories.get(delegatorName);
		// the sessionFactory haven't init before
		if (sessionFactory == null) {
			Debug.logInfo("building hibernate SessionFactory ...", MODULE);
			...
			Debug.logInfo("groupHelperName : " + groupHelperName, MODULE);
			...
			String filePath = HIBERNATE_CFG_PATH + datasourceName + HIBERNATE_CFG_EXT;
			Debug.logInfo("loaing hibernate configure file: " + filePath, MODULE);
			annotationConfiguration.configure(filePath);
			long startTime = Calendar.getInstance().getTimeInMillis();
			sessionFactory = annotationConfiguration.buildSessionFactory();
			long endTime = Calendar.getInstance().getTimeInMillis();

			Debug.logInfo("buildSessionFactory, time cost:" + (endTime - startTime) / 1000 + "s", MODULE);
			Debug.logInfo(sessionFactory.getStatistics().toString(), MODULE);

			Debug.logVerbose("listing loaded entities ...", MODULE);
			// iterator all classes which are success load of hibernate, it just
			// only for debug use.
		...
		Debug.logInfo(" hibernate SessionFactory size:" + sessionFactories.size(), MODULE);
		return sessionFactory;
	}

	/**
	 * Gets the Hibernate <code>Session</code> object, it return an open session
	 * from the getSessionFactory using the delegator already in this
	...
	public Session getSession() throws InfrastructureException {
		...
		Debug.logInfo("getSession, session.isOpen()=" + session.isOpen(), MODULE);
		return session;
	}

	/**
	 * Gets the helper name, which is used to map a datasource to a set of
	 * entities in the ofbiz entity engine. Currently this just returns the
	 * static HELPER_NAME defined in the class, but it's here in case later we
	 * need more sophisticated ways to get the helper name.
	 * 
	 ...
	 * 
	 * @param dispatcher
	 *            a <code>LocalDispatcher</code> value
	...
	/**
	 * Loads the system user login for Repositories and Factories that use this
	 * Infrastructure, plus all their sub-classes.
	 */
	...
	/**
	 * Gets a configuration value from the database. If it's not configured, use
	 * user supplied defaultValue, then OpentapsConfigurationType.defaultValue
	 * 
	 * @param configTypeId
	 *            the config type to get
	 * @param defaultValue
	 *            the value to return if no configuration is set
	 * @return a <code>String</code> value
	 * @exception InfrastructureException
	 *                if an error occurs
	 */
		...
		try {
			...
			if (configuration == null) {
				if (defaultValue != null) {
					...
					Debug.logWarning("No value found for configuration [" + configTypeId + "] returning a default of [" + defaultValue
							+ "]", MODULE);
					return defaultValue;
				} else {
					// use the default value of the configuration type
					OpentapsConfigurationType configurationType = (OpentapsConfigurationType) session.get(OpentapsConfigurationType.class,
							configTypeId);
					if (configurationType != null) {
						Debug.logWarning("No value found for configuration [" + configTypeId + "] returning a default of ["
								+ configurationType.getDefaultValue() + "]", MODULE);
	...
	/**
	 * Gets a configuration value as boolean from the database with null default
	 * value. This is a convenience method to get the common Y/N flags
	 * configuration values.
	 * 
	 * @param configTypeId
	 *            the config type to get
	 * @return <code>TRUE</code> if the config value is Y/y, <code>FALSE</code>
	 *         otherwise (including for null values)
	 * @exception InfrastructureException
	 *                if an error occurs
	 */
	 ...
	 *            the value to return if no configuration is set
	 * @return <code>TRUE</code> if the config value is Y/y, <code>FALSE</code>
	 *         for any other non empty values, <code>defaultValue</code> if no
	 *         configuration set
	 * @exception InfrastructureException
	 *                if an error occurs
	 */
	...
	/**
	 * Sets the configuration value for a configTypeId inside its own
	 * transaction.
	 * 
	 * @param configTypeId
	 *            a <code>String</code> value
	...
	public void setConfigurationValue(String configTypeId, String value, String comments) throws InfrastructureException {
		...
		// saveOrUpdate will try to update when id not null, so we should use
		// save to instead of it.
	 ...
	 * @param id
	 *            a <code>Serializable</code> instance
	 */
	 ...
	 *            a <code>SessionFactory</code> instance
	 * @param persistentClass
	 *            a <code>Class</code> instance
	 * @param id
	 *            a <code>Serializable</code> instance
