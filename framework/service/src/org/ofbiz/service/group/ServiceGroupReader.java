...
public class ServiceGroupReader {
	...
	// using a cache is dangerous here because if someone clears it the groups
	// won't work at all: public static UtilCache groupsCache = new
	// UtilCache("service.ServiceGroups", 0, 0, false);
	...
	public static void readConfig() {
		...
		for (Element serviceGroupElement : UtilXml.childElementList(
				rootElement, "service-groups")) {
		...
		// get all of the component resource group stuff, ie specified in each
		// ofbiz-component.xml file
		for (ComponentConfig.ServiceResourceInfo componentResourceInfo : ComponentConfig
				.getAllServiceResourceInfos("group")) {
	...
	public static void addGroupDefinitions(ResourceHandler handler) {
		...
		for (Element group : UtilXml.childElementList(rootElement, "group")) {
			...
			// can lazyload GroupModel when call liura 2013-09-21
		...
		if (Debug.importantOn()) {
			...
			Debug.logImportant("Loaded [" + numDefs
					+ "] Group definitions from " + resourceLocation, module);
		}
	}
