...
public abstract class PartyListView extends EntityListView {
	...
	/**
	 * Constructor giving a title for this list view, which is displayed in the
	 * UI.
	 * 
	 * @param title
	 *            the title label for this list view.
	 */
	...
	/**
	 * Placeholder to remind extended classes that on of the init methods must
	 * be called.
	 */
	public abstract void init();

	/**
	 * Configures the list columns and interaction with the server request that
	 * populates it. Constructs the column model and JSON reader for the list
	 * with the default columns for Party and extra columns.
	 * 
	 * @param entityFindUrl
	 *            the URL of the request to populate the list
	 * @param partyIdLabel
	 *            the label of the ID column, which depends of the entity that
	 *            is listed
	 * @param nameColumns
	 *            the list of extra columns to insert after the ID column, in
	 *            the order they should appear. By default the party list
	 *            provides ID, Country, State, City, Postal code
	 */
	 ...
	 * Configures the list columns and interaction with the server request that
	 * populates it. Constructs the column model and JSON reader for the list
	 * with the default columns for Party and extra columns, as well as a link
	 * for a view page.
	 * 
	 * @param entityFindUrl
	 *            the URL of the request to populate the list
	 * @param entityViewUrl
	 *            the URL linking to the entity view page with a placeholder for
	 *            the ID. The ID column will use it to provide a link to the
	 *            view page for each record. For example
	 *            <code>/crmsfa/control/viewContact?partyId={0}</code>. This is
	 *            optional, if <code>null</code> then no link will be provided
	 * @param partyIdLabel
	 *            the label of the ID column, which depends of the entity that
	 *            is listed
	 * @param nameColumns
	...
	protected void init(String entityFindUrl, final String entityViewUrl, String partyIdLabel, String[] nameColumns) {
		...
		ColumnConfig columnPhone = makeLinkColumn(UtilUi.MSG.partyPhoneNumber(), idDefinition, new StringFieldDef(
				PartyLookupConfiguration.INOUT_FORMATED_PHONE_NUMBER), entityViewUrl, true);
		columnPhone.setRenderer(new Renderer() {
			public String render(Object value, CellMetadata cellMetadata, Record record, int rowIndex, int colNum, Store store) {
				...
				if ("Y".equals(voipEnabled)) {
					String link = "<a class=\"linktext\" href=\"javascript:opentaps.makeOutgoingCall('" + primaryCountryCode + "','"
							+ primaryAreaCode + "','" + primaryContactNumber + "');\">" + formatedPrimaryPhone + "</a>";
					return link;
				} else {
		...
		ColumnConfig columnEmail = makeLinkColumn(UtilUi.MSG.partyEmailAddress(), idDefinition, new StringFieldDef(
				PartyLookupConfiguration.INOUT_EMAIL), entityViewUrl, true);
		columnEmail.setRenderer(new Renderer() {
			public String render(Object value, CellMetadata cellMetadata, Record record, int rowIndex, int colNum, Store store) {
				...
				if (contactMechIdTo == null || "".equals(contactMechIdTo)) {
					...
					String url = "<a class=\"linktext\" href='writeEmail?contactMechIdTo=" + contactMechIdTo + "&internalPartyId="
							+ internalPartyId + "&donePage=" + viewAction + "'>" + email + "</a>";
		...
		setColumnHidden(PartyLookupConfiguration.INOUT_LAST_NAME, true);
	...
	/**
	 * Set <code>ignoreLinkColumn</code> value. Use when we don't need
	 * hyperlinks in ID or name columns, e.g. the list view is embed in another
	 * GWT window and you want to handle selection in listener.
	 * 
	 * @param flag
	 *            new value
	 */
	...
	/**
	 * Filters the records of the list by showing only those belonging to the
	...
	/**
	 * Filters the records of the list by party id matching the given sub
	 * string.
	 * 
	 * @param partyId
	...
	/**
	 * Filters the records of the list by party classification matching the
	...
	/**
	 * Filters the records of the list by address of the party matching the
	 * given sub string.
	 * 
	 * @param address
	 *            a <code>String</code> value
	...
	/**
	 * Filters the records of the list by country of the party matching the
	 * given country id.
	 * 
	 * @param countryGeoId
	 *            a <code>String</code> value
	...
	/**
	 * Filters the records of the list by state / province of the party matching
	 * the given state / province id.
	 * 
	...
	/**
	 * Filters the records of the list by city of the party matching the given
	 * sub string.
	 * 
	 * @param city
	 *            a <code>String</code> value
	...
	/**
	 * Filters the records of the list by postal code of the party matching the
	 * given sub string.
	 * 
	 * @param postalCode
	 *            a <code>String</code> value
	...
	/**
	 * Filters the records of the list by phone country code of the party
	 * matching the given sub string.
	 * 
	 * @param countryCode
	 *            a <code>String</code> value
	 */
	...
	/**
	 * Filters the records of the list by phone area code of the party matching
	...
	/**
	 * Filters the records of the list by phone number of the party matching the
	 * given sub string.
	 * 
	 * @param number
	 *            a <code>String</code> value
	...
	/**
	 * Filters the records of the list by the to name of the party matching the
	 * given sub string.
	 * 
	 * @param toName
	 *            a <code>String</code> value
	...
	/**
	 * Filters the records of the list by the attention name of the party
	 * matching the given sub string.
	 * 
	 * @param attnName
	 *            a <code>String</code> value
	...
	/**
	 * Filters the records of the list by email of the party matching the given
	 * sub string.
	 * 
	 * @param number
	 *            a <code>String</code> value
