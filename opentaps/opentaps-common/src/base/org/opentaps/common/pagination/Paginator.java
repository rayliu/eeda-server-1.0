...
/**
 * The Paginator is a session object which manages a PaginationState and a
 * ListBuilder. It is responsible for interpreting actions such as sorting a
 * list, getting a page and saving the state. This is a generic object designed
 * to be called from generic requests.
 * 
 * The idea is that the developer only needs to worry about defining a
 * ListBuilder for the Paginator. There are conveniences for creating
 * ListBuilders, which simlify the whole thing even more.
 * 
...
public class Paginator implements HttpSessionBindingListener {
	...
	protected Map params; // parameters that are passed to each pagination
							// request as specified in the varargs for
							// <@paginate> macro
	protected boolean isFormlet;
	protected long cachedSize = -1; // cached size for external calls to the get
									// size functions
	 ...
	 */
	public Paginator(String paginatorName, ListBuilder listBuilder, Map params,
			GenericValue userLogin, String applicationName,
			boolean rememberPage, boolean rememberOrderBy,
			boolean renderExcelButton) {
		this.paginatorName = paginatorName;
		this.listBuilder = listBuilder;
		setParams(params);
		isFormlet = false;
		state = new PaginationState(paginatorName, userLogin, applicationName,
				rememberPage, rememberOrderBy);
	...
	/**
	 * Change the list builder and clean up any resources used by old list
	 * builder.
	 */
	...
	private long getLastPageCursorIndex() {
		try {
			if (listBuilder.getListSize() == 0 || state.getViewAll())
				return 0;
			...
			return state.getViewSize()
					* (listBuilder.getListSize() / state.getViewSize());
		} catch (ListBuilderException e) {
			Debug.logError(e, e.getMessage(), module);
			return 0;
		}
	...
	// pattern is to set and save state first, which is enforced by this
	// function
	...
	/**
	 * Gets the last page, which has a cursor position an integer multiple of
	 * the viewSize.
	 */
	...
	/**
	 * Changes the size of the viewport and sents the state back to the first
	 * page.
	 */
	...
	public void changeOrderBy(List orderBy) {
		if (getListSize() == 0)
			return;
		if (orderBy == null && state.getOrderBy() == null)
			return;
	...
	/**
	 * Changes the order and sets the state back to the first page. Using the
	 * given reverse orderBy list instead of computing from the normal orderBy.
	 */
	...
	/**
	 * Reverses the order of the sort by adding or removing DESC to the first
	 * parameter.
	 */
