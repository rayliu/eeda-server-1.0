 ...
 * <ul>
 * <li>the <code>createHibernateSearchIndex</code> service regenerates the
 * search index for all entities configured in
 * <code>entitysearch.properties</code>
 * <li>the <code>createIndexForGenericEntity</code> service updates the search
 * index with the values of an entity set by <code>setValue</code>
 * </ul>
 */
public class IndexingService extends Service implements
	...
	public void createHibernateSearchIndex() throws ServiceException {
		try {
			Properties entitySearchProperties = UtilProperties
					.getProperties("entitysearch.properties");
			...
			while (enumeration.hasMoreElements()) {
				...
				// find and create index for each entity define in
				// entitysearch.properties
				if ("index".equals(value)) {
					if (!key.contains(".")) {
						key = "org.opentaps.base.entities." + key;
					}
					Debug.logInfo("creating index for entity [" + key + "]",
							MODULE);
	...
	/**
	 * Creates the hibernate search index for the given none-view entity
	 * classes.
	 * 
	 * @param classes
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void createHibernateSearchIndex(Class[] classes)
			throws ServiceException {
		...
		try {
			...
			for (int i = 0; i < classes.length; i++) {
				...
				if (!object.isView()) {
					Debug.logVerbose(
							"refresh index for [" + cls.getCanonicalName()
									+ "] begin.", MODULE);
			...
			if (tx != null) {
				try {
					...
					Debug.logWarning(
							e2,
							"Could not rollback the hibernate transaction on error.",
							MODULE);
				}
			}
			throw new ServiceException(e);
			...
			try {
				...
				Debug.logWarning(e, "Could not close the FullTextSession.",
						MODULE);
			}
			...
			Debug.logInfo("Session closed", MODULE);
		}
	}
	 ...
	 *            a <code>FullTextSession</code> value
	 * @param entityClass
	 *            a <code>Class</code> value
	 */
	@SuppressWarnings("unchecked")
	private void createIndexForEntity(FullTextSession fullTextSession,
			Class entityClass) {
		Criteria query = fullTextSession
				...
				// set flush mode, ensure it will write to disk on commit the
				// transaction
	...
	public void createIndexForGenericEntity() throws ServiceException {
		...
		try {
			...
			Class cls = Class.forName("org.opentaps.base.entities."
					+ entityName);
			Serializable id = null;

			if (pkFieldNames.size() > 1) {
				// multi fields pk
				Class pkCls = Class.forName("org.opentaps.base.entities."
						+ entityName + "Pk");
				id = (Serializable) pkCls.newInstance();
				for (String pkFieldName : pkFieldNames) {
					HibernateUtil.setFieldValue(id, pkFieldName,
							value.get(pkFieldName));
				}
			...
			Debug.logInfo("createIndexForGenericEntity: got id [" + id
					+ "] for entity: " + entityName, MODULE);
			if (id != null) {
			...
			fullTextSession.clear();
			tx.commit();
			Debug.logInfo("createIndexForGenericEntity: index committed",
					MODULE);
			if (session.isDirty()) {
