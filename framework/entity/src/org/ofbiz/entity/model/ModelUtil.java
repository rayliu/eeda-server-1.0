...
public class ModelUtil {
	...
	public static Cache getEhcache() {
		// if (CacheManager.getInstance().getCache("eedaCache") == null) {
		// // Create a singleton CacheManager using defaults
		// CacheManager manager = CacheManager.create();
		// int maxEntriesLocalHeap = 200;
		// // Create a Cache specifying its configuration.
		// Cache eedaCache = new Cache(new CacheConfiguration("eedaCache",
		// maxEntriesLocalHeap)
		...
		// manager.addCache(eedaCache);
		// }
		return CacheManager.getInstance().getCache("eedaEntityCache");
	}
	...
	/**
	 * Converts a database name to a Java class name. The naming conventions
	 * used to allow for this are as follows: a database name (table or column)
	 * is in all capital letters, and the words are separated by an underscore
	 * (for example: NEAT_ENTITY_NAME or RANDOM_FIELD_NAME); a Java name (ejb or
	 * field) is in all lower case letters, except the letter at the beginning
	 * of each word (for example: NeatEntityName or RandomFieldName). The
	 * convention of using a capital letter at the beginning of a class name in
	 * Java, or a lower-case letter for the beginning of a variable name in Java
	 * is also used along with the Java name convention above.
	 * 
	 * @param columnName
	 *            The database name
	...
	/**
	 * Converts a database name to a Java variable name. The naming conventions
	 * used to allow for this are as follows: a database name (table or column)
	 * is in all capital letters, and the words are separated by an underscore
	 * (for example: NEAT_ENTITY_NAME or RANDOM_FIELD_NAME); a Java name (ejb or
	 * field) is in all lower case letters, except the letter at the beginning
	...
	/**
	 * Converts a Java variable name to a database name. The naming conventions
	 * used to allow for this are as follows: a database name (table or column)
	 * is in all capital letters, and the words are separated by an underscore
	 * (for example: NEAT_ENTITY_NAME or RANDOM_FIELD_NAME); a Java name (ejb or
	 * field) is in all lower case letters, except the letter at the beginning
	...
	/**
	 * Start by removing all vowels, then pull 1 letter at a time off the end of
	 * each _ separated segment, go until it is less than or equal to the
	 * desired length
	 * 
	 * @param dbName
	 * @param desiredLength
	...
	public static String shortenDbName(String dbName, int desiredLength) {
		...
		if (dbBuf.length() > desiredLength) {
			...
			for (int i = dbBuf.length() - 1; i > 0; i--) {
				// don't remove vowels that are at the beginning of the string
				// (taken care of by the i > 0) or right after an underscore
		...
		while (dbBuf.length() > desiredLength) {
			...
			while (usIndex > 0 && dbBuf.length() > desiredLength) {
				// if this is the first word in the group, don't pull letters
				// off unless it is 4 letters or more
				int prevUsIndex = dbBuf.lastIndexOf("_", usIndex - 1);
				if (prevUsIndex < 0 && usIndex < 4) {
					break;
				}

				// don't remove characters to reduce the size two less than
				// three characters between underscores
				if (prevUsIndex >= 0 && (usIndex - prevUsIndex) <= 4) {
					usIndex = prevUsIndex;
					continue;
				}

				// delete the second to last character instead of the last,
				// better chance of being unique
			...
			if (dbBuf.length() > desiredLength) {
				...
				// don't remove characters to reduce the size two less than two
				// characters between underscores
		...
		while (dbBuf.length() > desiredLength) {
			...
			// don't remove the first segment, just remove the second over and
			// over until we are short enough
			int firstUs = dbBuf.indexOf("_");
			if (firstUs > 0) {
				int nextUs = dbBuf.indexOf("_", firstUs + 1);
				if (nextUs > 0) {
					// Debug.logInfo("couldn't shorten enough normally, removing second segment from "
					// + dbBuf, module);
		...
		// Debug.logInfo("Shortened " + dbName + " to " + dbBuf.toString(),
		// module);
		return dbBuf.toString();
	}

	/**
	 * Converts a package name to a path by replacing all '.' characters with
	 * the File.separatorChar character. Is therefore platform independent.
	 * 
	 * @param packageName
	 *            The package name.
	...
	public static String packageToPath(String packageName) {
		// just replace all of the '.' characters with the folder separater
		// character
	...
	public static String induceFieldType(String sqlTypeName, int length, int precision, ModelFieldTypeReader fieldTypeReader) {
		...
		if (sqlTypeName.equalsIgnoreCase("VARCHAR") || sqlTypeName.equalsIgnoreCase("VARCHAR2")
				|| (sqlTypeName.equalsIgnoreCase("CHAR") && length > 1)) {
			if (length <= 10)
				return "very-short";
			if (length <= 60)
